                        ...
                        if is_stubbed {
                            ...
                            if config.ignore_processes.iter().any(|p| p == &proc_name) {
                                ...
                                thread::spawn(move || {
                                    ...
                                    if is_primary {
                                        // Capture timestamps before the kernel overwrites them during the fill
                                        ...
                                        if restored {
                                            ...
                                            // 1. Restore the original timestamps
                                            let c_path = std::ffi::CString::new(path_clone.clone()).unwrap();
                                            let times = [
                                            ...
                                            // 2. Re-queue for the Janitor so it can be stubbed again later
                                            if let Ok(local_conn) = rusqlite::Connection::open(&db_path_clone) {
        ...
        match rx.recv_timeout(Duration::from_secs(3600)) {
            Ok(mut path_str) => {
                ...
                if !is_stubbed {
                    ...
                    // Optimization: Skip tape write if file hasn't changed (e.g. it was just restored by an indexer)
                    let mut needs_archive = true;
                    if let Ok((db_mtime, db_size)) = conn.query_row(
                        "SELECT original_mtime, payload_size FROM catalog WHERE file_path = ?1 ORDER BY version DESC LIMIT 1",
                        params![&path_str],
                        |row| Ok((row.get::<_, i64>(0)?, row.get::<_, i64>(1)?))
                    ) {
                        if db_mtime == meta.mtime() && db_size == meta.len() as i64 {
                            needs_archive = false;
                            info!("[Worker] File unmodified since last archive. Skipping write, re-stubbing directly.");
                        }
                    }
                    ...
                    if needs_archive {
                        ...
                        match archive_file(&conn, &path_str, &config, use_direct_io) {
                            Ok((replica_list, jump_table)) => {
                                ...
                                if !db_commit_success {
                                    error!("[Worker] Catalog write failed for {}. Aborting stub process.", path_str);
                                    continue;
                                }
                                ...
                                if post_archive_touch != pre_archive_touch {
                                    error!("[Worker] File {} modified during archive! Aborting stub to preserve data.", path_str);
                                    continue; 
                                }
                                ...
                                let replicas_json: Vec<serde_json::Value> = replica_list.iter().map(|(offset, _, _, _, _, tape_uuid, dev_path, _)| {
                                    json!({ "tape_uuid": tape_uuid, "tape_offset": offset, "device_path": dev_path })
                                }).collect();
                                ...
                                sidecar.send_event(json!({
                                    ...
                                    "payload_size": payload_size_saved,
                                    "blake3_hash": &replica_list[0].4,
                        ...
                        // File was unmodified (e.g. indexer restore). Stub it instantly without hitting tape.
                        if let Err(e) = stub_file(&path_str, meta.len()) {
                            error!("[Worker]  Failed to stub {}: {}", path_str, e);
                        } else {
                            conn.execute("DELETE FROM active_tracking WHERE file_path = ?1", params![&path_str]).unwrap();
                            info!("[Worker] Re-stubbed unmodified file and removed from queue.");
                        }
                    }
                } else {
                    conn.execute("DELETE FROM active_tracking WHERE file_path = ?1", params![&path_str]).unwrap();
...
pub fn run_janitor_scanner(tx: mpsc::SyncSender<String>, config: Arc<HuskConfig>) {
    ...
    for (path_str, last_touch) in rows {
        ...
        // Debounce: Ensure file hasn't been written to in the last 5 seconds
        let is_settled = age_secs >= 5;
        let mut should_archive = is_settled && (is_immediate || age_secs >= max_age_secs);
